...
Design patterns are best practices to recurring design problems, first it was proposed by the Gang of Four in their seminal book.

(Some study author) have been able to show that the presence of design patterns,
if correctly presented, can help developers better understand a piece of
...
developers in understanding a piece of software through reverse engineering and facilitate work during maintenance.
...
Systematic mapping is a relatively new research method in software
engineering, adapted from other disciplines by Kitchenham
[5]. It is an alternative to systematic reviews and could be used if
the amount of empirical evidence is too little, or if the topic is too
broad, for a systematic review to be feasible. A mapping study is performed
at a higher granularity level with the aim to identify research
gaps and clusters of evidence in order to direct future research.
Petersen et al. [1] describe how to conduct
a systematic mapping study in software engineering. Our study
is conducted in accordance with these guidelines and 
we have introduced a classification scheme specific to our topic.
...
It is organized as follows: 
 ...
 and it concludes with related works. 
...
classification scheme by individual reviewers if considered relevant to the
study.

Misleading articles that passed screening phase were removed at that point.
A total of 39 articles have been rejected in this fashion leaving a total of
55 articles relevant to our study

The resulting classification and interpretation will be discussed in the next
section.

\section{STUDY RESULTS}

In this section, the article will analyse different measures obtained from
the final set of articles.
More precisely, the article will look at the yearly distribution, the publisher
distribution, the publication type distribution and each category define
earlier in the classification scheme.
...
\begin{figure}
  \centering
  \includegraphics[scale=0.40]{publisher.png}
  \caption{Distribution of papers by publishers}
\end{figure}
...
\subsection{Publishers Distribution}

\textit{Fig. 4} shows a partial distribution of the articles between their
resspective publishers. 
In many cases, data exported from Scopus is lacking the publisher which
resulted in the classification of many articles as \textit{Other}.
Manual inspection would be required.

\subsection{Publication type distribution}

\textit{Fig. 5} shows the distribution of the papers according to their
types.
Unsurprisingly, half of them are conference paper, while a third are journal
articles.
...
Another big avenue for detection strategy lies in artificial intelligence
and machine learning.
In fact, a fifth of the articles make use an AI technique.

Graph methods are popular popular and half the time have been used jointly with
...
Choosing a specific type of analysis tels a lot about the associated technique.
Static analysis is useful when trying to extract structural and semantic
information about a program while dynamic analysis makes it easier to get
information about the behaviour of an algorithm.

The analysis type also affect the kind of tool the algorithm is implemented
in.
In fact dynamic analysis requires much more processing time due to the
profiling aspect associated with it.
This effectively prevent the inclusion of detection algorithm as a realtime
programing aid.

A little bit less than half of the articles favoured a static approach to the
recognition of design motifs, while the other half had a dynamic aspect to
...
patterns detected. \textit{Fig. 7} and \textit{Fig. 8}
...
For structural patterns, a static analysis can be said to be efficient enough.
More than half the detection method used such an analysis with good results

For behavioural pattern, static analysis migh be sufficient but a majority of
article includes a dynamic component to the anaylis.
Tsantalis et al.\cite{6} had good result with their graph similarity scoring
approach for detecting structural and behavioural pattern with a static method.

Kaczor et al.\cite{4} use an hybrid graph matching algorithm to detect creationnal
and structural design pattern.

Creationnal patterns show an interesting distribution of the analysis types.
In fact, it could be said that their identification is difficult with
 a dynamic analysis alone.
Strangely though, a high proportion of article report the recognition of
creationnal design patterns with only a static analysis.
...
\bibitem{c2} Landis, J. Richard, and Gary G. Koch. "The measurement of observer agreement for categorical data." biometrics (1977): 159-174.


\bibitem{C3} D. Yu, Y. Zhang and Z. Chen. "A comprehensive approach to the recovery of design pattern instances based on sub-patterns and method signatures" 2015, Journal of Systems and Software


\bibitem{C4} O. Kaczor, Y. Guéhéneuc and S. Hamel "Identification of design motifs with pattern matching algorithms" 2010, Information and Software Technology


\bibitem{C5} B.A. Kitchenham, Guidelines for performing systematic literature reviews in
software engineering version 2.3, Technical Report S.o.C.S.a.M. Software
Engineering Group, Keele University and Department of Computer Science
University of Durham, 2007.

\bibitem{C6} Tsantalis, N. Chatzigeorgiou, A. Stephanides, G. 
Halkidis, S.T., "Design pattern detection using similarity scoring" 2006, IEEE Transactions on Software Engineering

\bibitem{C7} 

\bibitem{c3} www.scopus.com
